...
=======
-- ============================================================
-- UDAAN Carpooling Platform — Database Schema
-- MySQL 8.x / InnoDB — Full schema per Section 5.2
-- ============================================================
...
CREATE TABLE `rides` (
    ...
    `travel_date`     DATE NOT NULL,
    `travel_time`     TIME NOT NULL,
    `available_seats` TINYINT UNSIGNED NOT NULL DEFAULT 3,
    `total_seats`     TINYINT UNSIGNED NOT NULL DEFAULT 3,
    `fare_per_seat`   DECIMAL(8,2) NOT NULL DEFAULT 0.00,
...
-- ── Bookings (also carries trip lifecycle status) ─────────
DROP TABLE IF EXISTS `bookings`;
CREATE TABLE `bookings` (
    `id`                INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    `ride_id`           INT UNSIGNED NOT NULL,
    ...
    `status`            ENUM('booked','trip_started','trip_in_progress','trip_completed','payment_pending','payment_completed','cancelled')
                        NOT NULL DEFAULT 'booked',
    `booked_at`         TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    `trip_started_at`   TIMESTAMP NULL,
    `trip_completed_at` TIMESTAMP NULL,
...
-- ── Trip Locations (live tracking — one row per active ride) ──
DROP TABLE IF EXISTS `trip_locations`;
CREATE TABLE `trip_locations` (
    `id`          INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    `ride_id`     INT UNSIGNED NOT NULL UNIQUE,
    ...
    `eta_minutes` SMALLINT UNSIGNED NULL,
    `updated_at`  TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    CONSTRAINT `fk_triploc_ride` FOREIGN KEY (`ride_id`) REFERENCES `rides`(`id`)
        ON UPDATE CASCADE ON DELETE CASCADE
) ENGINE=InnoDB;
...
CREATE TABLE `wallet_transactions` (
    ...
    `type`       ENUM('recharge','debit') NOT NULL,
    `amount`     DECIMAL(10,2) NOT NULL,
    `reference`  VARCHAR(100) NULL COMMENT 'Razorpay payment id or booking ref',
    `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE `payments` (
    ...
    `booking_id`          INT UNSIGNED NOT NULL,
    `amount`              DECIMAL(8,2) NOT NULL,
    `method`              ENUM('cash','card','upi','wallet') NOT NULL DEFAULT 'wallet',
    `razorpay_payment_id` VARCHAR(100) NULL,
    `status`              ENUM('pending','success','failed') NOT NULL DEFAULT 'pending',
    `paid_at`             TIMESTAMP NULL,
